...
String AxisName = "BlackAxis2"; // Name your MADEaxis
...
bool debounce;
long debounceMillis; // For top switch to avoid debounce blocking coms
long debounceThreshold = 200;
bool follow;
...
{
  long elapsed = millis();
 if ((elapsed - debounceMillis) > debounceThreshold)
 {
  debounce= false;
 }
}
...
void UdpRead()
{
  ...
  String snapBoth = concatName + "/snapBoth";
    ...
    {     
      //Serial.println(msg.getInt(0));
        ...
        {          
         ...
         if (msg.fullMatch(snapBoth.c_str()))  // slider, slider target, followMode, led not added yet
        {          
          
          setLED(msg.getInt(3));
          SnapBoth(msg.getInt(0), msg.getInt(1), msg.getInt(2) );//, msg.getBoolean(2)); // added follow mode bool and led in 
        } 
        /////receive steppedMode
...
void SnapBoth(int valOne, int valTwo, int followInt)//, bool follow)
{
 
  if (followInt ==1 )
  {
    sliderOneTarget = valOne;
    moveOne = true;
    sliderOneStart = millis();
    FollowReceive(2,1);
    //setLED(0);
...
void readButtons()
{
  ...
  int readTopSwitch = !digitalRead(topSwitch);
  if (readTopSwitch != button2 && !debounce) //== LOW)
  {
    button2 = readTopSwitch;
    debounce = true;
