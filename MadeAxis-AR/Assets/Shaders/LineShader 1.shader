			...
			CGPROGRAM
				...
				FS_INPUT VS_Main(VS_INPUT v)
				{
					...
					float3 normalisedPosition = float3(
						normaliseValue(v.position.x, _MinNormX, _MaxNormX ,-0.5, 0.5),
						normaliseValue(v.position.y, _MinNormY, _MaxNormY ,-0.5, 0.5),
						normaliseValue(v.position.z, _MinNormZ, _MaxNormZ ,-0.5, 0.5));

//					output.pos = ObjectToWorldDistort3d(v.position);// normalisedPosition);
					//output.pos = ObjectToProjectionDistort(normalisedPosition);
					output.pos = mul(UNITY_MATRIX_VP, ObjectToWorldDistort3d(normalisedPosition));
					...
					//lookup brushing texture
					
					//brusedIndices[(int)v.normal.x] = 1.0;
					//}
					//else
					//{
					//brusedIndices[(int)v.normal.x] = 0.0;					
					//}
				...
				void GS_Main(point GS_INPUT p[1], inout TriangleStream<FS_INPUT> triStream)
				{
					...
					float halfS = 0.01f * _Size*brushSizeFactor;
